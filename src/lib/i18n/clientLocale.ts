...
//
// hanoba はバックエンドレス・静的サイト（殻は ja でビルド）。表示言語の切替は
// localStorage に保存した「ユーザーが選んだ言語」をクライアントで読むだけ＝Twitter モデル
// （URL は同じ・/en/ ルーティングは作らない）。サーバ（.astro）は触らない（単一責務）。
//
// auto-detect（navigator.language）はしない（段階2 のスコープ外）。既定は常に DEFAULT_LOCALE（ja）。
...
/** ユーザーが選んだ表示言語。localStorage 優先、無ければ既定（ja）。auto-detect はしない（段階2）。 */
