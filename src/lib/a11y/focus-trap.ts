 ...
 * - 末尾で Tab（!shift）→ 先頭へ。
 * - 先頭で Shift+Tab → 末尾へ。
 * - active がモーダル内の focusable に無い（外・未設定）→ 端へ引き込む（外へ逃がさない＝完全トラップ）。
 * - それ以外（中間）は null（ブラウザ既定の移動に任せる）。
 *
...
export function focusTrapTarget<T>(focusables: T[], active: T | null, shift: boolean): T | null {
  ...
  // active がモーダル内に無い場合は端へ吸い込み、フォーカスが外へ逃げるのを防ぐ。
  if (!focusables.includes(active as T)) return shift ? last : first;
  return null;
}
 ...
 * 注意: `position: fixed` な子孫は仕様上 offsetParent=null になり取りこぼす。
 *       現状のパネル内 focusable（ボタン）は static なので問題ないが、将来 fixed を足す場合は要見直し。
 */
