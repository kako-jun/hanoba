...
describe("citizenLevel", () => {
  ...
  it("L3: 名前＋投稿数 >= 15 ＋ 在籍 >= 30 日 で L3（条文ページまで・新キャップ 3・#469）", () => {
    expect(
      citizenLevel({ hasName: true, postCount: 15, earliestCreatedAt: NOW - 30 * DAY, now: NOW }),
    ).toBe(3);
  });
});
...
describe("citizenLevelFull（市民Ln 非キャップ・#272 段階2・複合 居住×投稿 AND）", () => {
  ...
  it("citizenLevel は citizenLevelFull を 3 で頭打ちにする（図鑑は L3＝条文まで・#469）", () => {
    const input = { hasName: true, postCount: 200, earliestCreatedAt: NOW - 500 * DAY, now: NOW };
    expect(citizenLevelFull(input)).toBeGreaterThan(3);
    expect(citizenLevel(input)).toBe(3);
  });
});
...
describe("citizenLevelLabel（旅人/市民の二値表示・#525＝#272 から方針転換）", () => {
  ...
  it("2 以上も数字を出さず市民のまま（Ln 表記は一切出さない）", () => {
    expect(citizenLevelLabel(2, "ja")).toBe("市民");
