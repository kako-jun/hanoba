...
//
// 市民手帳と同じ本ページャー（BookPager）を使うが、並び順は逆＝**最新記事が1ページ目**、
// 「次へ」で過去の記事へ辿る（BookPager 自体に特別な分岐は無く、単にページ番号の割り当てを
// 新→旧の順にしているだけ＝#164 で「特例分岐でなくデータの並びで表現する」設計）。
...
export interface GazetteArticle {
  id: string;
  /** 1-indexed のページ番号。1 が最新（BookPager と同じ形）。 */
  page: number;
...
/** 記事の言語非依存な骨格（新しい記事ほど先頭＝配列順がそのままページ順になる）。 */
const ARTICLE_KEYS = [
  {
...
/** 全記事（新しい順・1 が最新）を locale で組み立てる。 */
