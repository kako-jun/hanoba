...
describe("多肉植物・サボテン・メセンの人気属の検索到達性（#490）", () => {
  const terms = [
    "雅楽の舞", "神想曲", "イワレンゲ", "カゲツ", "ハートカズラ",
    "シャコバサボテン", "カニバサボテン", "バニーカクタス", "ノパル", "ミヌスクラ", "老楽",
    "マツバギク", "アイスプラント",
  ];
...
describe("観葉植物・草花・蘭・花木庭木の人気属の検索到達性（#492）", () => {
  const terms = [
    "オリヅルラン", "セントポーリア", "ペペロミオイデス", "ムラサキゴテン", "シマフィンレイシア",
    "インパチェンス", "ニューギニアインパチェンス", "マツバボタン", "ポーチュラカ", "ニチニチソウ",
    "ネモフィラ", "スイートピー", "フクシア", "マッカイ", "ノダフジ", "ヤマフジ",
  ];
  for (const term of terms) {
    it(`「${term}」が検索で 1 件以上ヒットする`, () => {
      expect(searchCatalog(VARIETY_CATALOG, term).length).toBeGreaterThan(0);
...
describe("水草カボンバ・果樹ヤマモモの検索到達性（#494）", () => {
  const terms = ["カボンバ", "ヤマモモ"];
  for (const term of terms) {
    it(`「${term}」が検索で 1 件以上ヒットする`, () => {
      expect(searchCatalog(VARIETY_CATALOG, term).length).toBeGreaterThan(0);
    });
