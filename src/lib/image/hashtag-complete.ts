 ...
 * - tag を正規化して語境界で一致する箇所を除去。直後の空白を1つ畳んで二重空白を残さない。
 * - 末尾は insertTag の規約（`#tag ` で終わる）に合わせ、タグが残るうちは末尾空白1つを保つ。
 * - **先頭/中間/末尾どのタグを外しても、タグが残るうちは散文とタグ群の区切り空行（`\n\n`）を保つ**
 *   （#282）。除去時に `#tag` の**直前**の空白は消費せず（境界はゼロ幅 lookbehind で担保）、**直後**の
 *   空白を1つだけ消費するので、`本文\n\n#A #B ` の先頭 `#A` を外しても区切りの `\n\n` を壊さない。
 *   かつ各タグが `#tag ` で末尾スペース付き（insertTag 規約）なので直後スペースを食えば二重空白も出ない。
...
export function removeTag(caption: string, tag: string): string {
  ...
  if (norm === "") return caption;
  const escaped = norm.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  // 直前の境界（先頭 or 空白）はゼロ幅 lookbehind で担保し**消費しない**（区切り `\n\n` を壊さない・
  // 語頭であることも担保）。語末は後続の空白/終端で担保し、**直後の空白/タブを1つだけ**飲む
  // （改行は飲まない＝タグ群末尾の `\n` を残す）。各タグは `#tag ` 末尾スペース付き（insertTag 規約）
  // なので直後スペースを食えば二重空白は生じず、無関係な散文の連続スペースには触れない。
  ...
  if (removed !== caption) {
    // 末尾の連続改行＋（前後の）空白・タブをまとめて畳む。先頭の `[ \t]*` は改行直前に残った
    // 単独スペース（行内タグを最後に外した残り `prose \n` 等）も飲み、散文だけに戻す。
    // タグが残るうちは末尾行が `#tag ` で `\n` で終わらないので、ここには掛からない（末尾空白1つを保つ）。
