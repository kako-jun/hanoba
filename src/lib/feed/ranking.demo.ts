...
// 目的は、まだ実投稿が少ない launch 直後でも、複数週・先週比（↑↓/NEW/RE）・sparkline といった
// リッチな状態を `npm run dev` でリレーに何も publish せず確認すること（捏造データを本番に出さない）。
//
// 票数・順位は意図的に作り込む（先週比 up/down/same・NEW・RE・sparkline の山谷が出るように）。
