...
describe("名前付きビューの状態（get/add/remove/rename・#139 段階3）", () => {
  beforeEach(() => {
  ...
  it("rename でラベルだけ付け替える（query/id は不変）", () => {
    const one = addSavedView("実生", "#アガベ");
    const id = one[0]!.id;
    const after = renameSavedView(id, "アガベ実生");
    expect(after[0]!.label).toBe("アガベ実生");
    expect(after[0]!.query).toBe("#アガベ");
    expect(after[0]!.id).toBe(id);
  });

  it("rename: 空ラベル・無い id は無視（現状を返す）", () => {
    const one = addSavedView("実生", "#アガベ");
    const id = one[0]!.id;
    expect(renameSavedView(id, "  ")).toEqual(one);
    expect(renameSavedView("nope", "x")).toEqual(one);
  });
