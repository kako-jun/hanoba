...
// 背景画像3枚に preload ヒントを足すようになった。#164 で gazette.astro（GazetteBook・同じ3枚を
// 共有する本ページャー BookPager）も同じ理由で slot="head" を使うようになったので、対象ページを
// 列挙する形へ一般化した。このリポには .astro を直接レンダリングするテストインフラが無い
//（vitest.config.ts に Astro Vite プラグイン未設定）ため、source-text / fs ベースの軽量テストで
// 配線・typo・漏れ出し・既定の空を静的に守る。
//
// パス文字列自体は cityHallAssets.ts の共有定数が正本（#484 should-2）。対象ページは
...
// 本ページャー（BookPager）を client:only で使い、この3枚を preload するページ（#164 で複数化）。
const BOOK_PAGES = ["about.astro", "gazette.astro"] as const;
...
describe.each(BOOK_PAGES)("%s の preload リンク（#484・#164）", (file) => {
  ...
  it('3枚それぞれへの rel="preload" as="image" リンクをちょうど1回ずつ持つ', () => {
    for (const [exportName] of PRELOAD_ASSETS) {
      expect(countPreloadLinks(src, exportName), exportName).toBe(1);
    }
  });

  it("cityHallAssets.ts の共有定数を import している（#484 should-2・二重ハードコード防止）", () => {
    expect(src).toMatch(/from ["']\.\.\/lib\/lore\/cityHallAssets\.ts["']/);
  });
});
...
describe('slot="head" の漏れ出し防止（#484・#164）', () => {
  // 本ページャー使用ページ（BOOK_PAGES）以外が同じ slot 名を使い出すと MainLayout 側の
  ...
  const otherPages = readdirSync(pagesDir, { recursive: true }).filter(
    (f): f is string =>
      typeof f === "string" && f.endsWith(".astro") && !(BOOK_PAGES as readonly string[]).includes(f),
  );

  it("対象ページ（本ページャー使用ページ以外）が見つかる（前提）", () => {
