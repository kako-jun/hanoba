...
// 検証する**方針なので、各テスト前に locale 設定（集約 blob `hanoba` の lang・#558）を ja に固定する。
// localStorage を自前で clear/設定するテスト（clientLocale.test の DEFAULT 検証など）は、自身の
// beforeEach がこの後に走って上書きするので干渉しない。他フィールドを種撒きするテストは
// appStorage.testutil の seedAppStorage（マージ書き込み）を使い、この lang:ja を潰さないこと。
beforeEach(() => {
  try {
    localStorage.setItem(APP_STORAGE_KEY, JSON.stringify({ lang: "ja" }));
  } catch {
