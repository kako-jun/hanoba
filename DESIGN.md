...
- **discover の多軸フィルタ（#24/#68/#131/#139 段階2・実装済）**: 「**誰の × どの品種(タグ) × いつ × 並び**」を同時に効かせる。状態は単一の `DiscoverFilter`（`{tags[], author, keyword, since, until, sort}`・`src/lib/feed/discoverFilter.ts`＝純粋ロジック）。
  - **入力経路**: 検索ボックスは `classifyDiscoverQuery` で `#アガベ`→tags／`葉焼け`→keyword／`npub1…`(=`nostr:`可)・`@名前`→author に振り分け（パワーユーザー向け）。構造化指定は折りたたみパネル `DiscoverFilterPanel`（品種/タグ複数チップ・投稿者 npub/@名前・期間 since〜until・並び 新着/古い/人気）。
  - **取得＝`fetchDiscoverFiltered`（client.ts に集約）**: author は `resolveAuthorPubkeys` で pubkey 群へ解決（npub は decode／`@名前`は NIP-50 kind:0 検索→`selectAuthorsByName` name 部分一致・最新・上限20）。relay には押せる軸（`kinds:1`/`authors`/`since`/`until`/`limit`）を base に乗せ、tags は**別名展開した `#t` の OR**（#23・「パキポ」→グラキリス等）＋ NIP-50 `"#tag"` 検索、keyword は NIP-50 検索＋同語 `#t`。制約軸（tags/author/keyword）が無ければ既定母集団 **#plantstr ∪ t:hanoba** を引く。取得後 `applyClientFilter` で**軸間 AND・軸内別名 OR・keyword 部分一致・期間・画像のみ**を確定し、`sortPosts` で並べる（popular は対象 id のいいね数を `fetchReactionCounts` で 1 回まとめ取り→降順）。allSettled で片肺 relay でも前進。読み取り側のみの強化で §6 契約（本文 `#` は `t` 化しない）は不変。
...
- **名前付きビュー（#139 段階3・実装済）**: discover のフィルタ状態を名前付きで保存し、上部チップでワンタップ切替する自分専用チャンネル。ビュー＝`{label, query}` を localStorage（`src/lib/feed/views.ts`・SSR 安全 getLS・配列順・同 query は二重登録しない・壊れ値は空配列に倒す）に保存し、`query` は **現在の filter の canonical 文字列（`serializeFilter`＝例 `tags=実生&sort=old`）を丸ごと**持つ＝多軸でもそのまま保存できる（views.ts は中身を解釈しない＝多軸化前の単一クエリ `#実生` も `parseFilterFromString` が後方互換で吸収）。UI `SavedViews`（チップ列＝`role=group`＋各チップ `aria-pressed`＝`FilterChips` に揃える。tabpanel 紐付け・roving tabindex を持たないため tab 契約は満たせない／「すべて」＝既定（空 filter）＋保存ビュー・active は現在 filter の `serializeFilter` 一致・保存/切替/削除。並べ替え UI は段階的に未実装）は DiscoverGrid の**filter 反映経路に乗せて切替**（チップ tap → その query を `applyView`＝`parseFilterFromString` で filter に戻して適用・`pushState`・戻るで前のビューへ／「すべて」は filter を空にして既定表示へ）。既定表示（空 filter＝すべて）は名前付きビューにできない（空 query は保存不可）。
