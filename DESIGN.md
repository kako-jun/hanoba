...
- **アイコン**: 寄せ集めの Unicode 記号（♡/×）を廃し、単一出自・統一線幅の SVG（`src/components/ui/Icon.tsx`）。**いいねは黄色い花（`flower`＝`text-ha-yellow`・#116。ハート/絵文字でなく SVG）**。投稿コンポーザの送信ボタンは綿毛（`dandelion`＝写真を風に放つ・#148。ナビ等の発芽双葉 `sprout`＝育てる世界観・#48 とは別アイコン）。撮影/アルバムは `camera`/`image`（#29）。情報ページ（About）導線は `info`(円＋i・#106)。サービスリンク（#35 Piece 2）も同じ線スタイルで `link`(地球)/`code`/`x`/`youtube`/`instagram`/`writing`/`art`/`music`/`shopping`/`game`/`at`/`chat` をカテゴリ単位で持つ（ブランドロゴは作らない）。**塗りの例外は `flower`(いいね)・`heart`(Ko-fi)・`x`（X 公式ロゴ・#115）・`github`（公式 Octocat・#118）**＝ブランド識別性が高いものは線縛り（#21）の明示的例外として公式グリフを塗りで使う。**ヘッダとフッタのナビは同じ並び（About→みんなの植物→あなたの植物→投稿する）に揃え、About には `info` を付ける（#106）。人気ランキング（`/ranking`・#162）はフッタのみに控えめに置く（みんなの植物の隣・共有ナビは増やしすぎない）。フッタ外部リンクは mypace=公式マーク画像 `public/mypace-icon.webp`（汎用★記号でなく mypace 独自の丸み星・#105）/GitHub=`github`（#105）。インラインのアイコン付きリンクは `leading-none` で行高を詰め、`items-center` でアイコンがテキストと光学的に揃う（#105 follow-up）。**
...
- **discover の多軸フィルタ（#24/#68/#131/#139 段階2・実装済）**: 「**誰の × どの品種(タグ) × いつ × 並び**」を同時に効かせる。状態は単一の `DiscoverFilter`（`{tags[], author, keyword, since, until, sort}`・`src/lib/feed/discoverFilter.ts`＝純粋ロジック）。
  - **入力経路**: 検索ボックスは `classifyDiscoverQuery` で `#アガベ`→tags／`葉焼け`→keyword／`npub1…`(=`nostr:`可)・`@名前`→author に振り分け（パワーユーザー向け）。構造化指定は折りたたみパネル `DiscoverFilterPanel`。**絞り込み関連 UI は全部この展開エリアに入れる**（kako-jun 指示）＝展開すると縦に〔保存した絞り込み（名前付き・最上部）→ 品種/タグ複数チップ・投稿者 npub/@名前・期間 since〜until・並び 新着/古い/人気 → この絞り込みを共有（最下部）〕。用語は「絞り込み」で統一（「ビュー」と呼ばない）。各コントロールは高さ `h-9` に揃え入力中/確定後で高さがブレない。日本語 IME の変換確定 Enter は `isComposing` ガードで確定操作に拾わない。期間の `<input type=date>` は `.ha-date`（`color-scheme:dark`＋`font-family:inherit`）でダーク地に馴染ませる（ピッカーアイコンを明色・本文フォントに統一。区切り `/`↔`-` と桁順は OS/ブラウザのロケール依存でネイティブ制約・CSS では変えられない）。
  - **取得＝`fetchDiscoverFiltered`（client.ts に集約）**: author は `resolveAuthorPubkeys` で pubkey 群へ解決（npub は decode／`@名前`は NIP-50 kind:0 検索→`selectAuthorsByName` name 部分一致・最新・上限20）。**NIP-50 は記号を演算子扱いする**（search.nos.today は実測で `kako-jun` のハイフンを「除外」と解釈し0件・`kako` だけなら多数ヒット）ため、relay へ渡す検索語は英数字・CJK 以外を空白に均し（`kako-jun`→`kako jun`）、厳密な name 一致は**元の name** で client 側（selectAuthorsByName）に取る。relay には押せる軸（`kinds:1`/`authors`/`since`/`until`/`limit`）を base に乗せ、tags は**別名展開した `#t` の OR**（#23・「パキポ」→グラキリス等）＋ NIP-50 `"#tag"` 検索、keyword は NIP-50 検索＋同語 `#t`。制約軸（tags/author/keyword）が無ければ既定母集団 **#plantstr ∪ t:hanoba** を引く。取得後 `applyClientFilter` で**軸間 AND・軸内別名 OR・keyword 部分一致・期間・画像のみ**を確定し、`sortPosts` で並べる（popular は対象 id のいいね数を `fetchReactionCounts` で 1 回まとめ取り→降順）。allSettled で片肺 relay でも前進。読み取り側のみの強化で §6 契約（本文 `#` は `t` 化しない）は不変。
  - **URL = filter の deep-link（#139 段階1/2）**: `?tags=トマト,実生&author=npub1…&q=葉焼け&since=2026-01-01&until=…&sort=old`（`serializeFilter`/`applyFilterToParams`。旧 `?q=`/`?tag=` は `parseFilter` が classify で吸収＝後方互換）。意図的操作（検索・パネル変更・タグクリック・ビュー切替）は `pushState`、復元（マウント/popstate）は `replaceState`/書き込み無し（ループ防止）。既定へ畳まれる空クリアは `replaceState`（戻る対象にしない）。`latestRef` で stale 応答を破棄。
  - **共有導線（#139 段階2・実装済）＝ `ShareFilter`**: 何か絞っている時だけ（`!isDefaultFilter`）「リンクをコピー」（`window.location.href` をクリップボードへ・ProfileEditor #213 と同じ作法＝成功で「コピーしました」2秒・失敗は黙る）と「X でシェア」（#37 の `openXShare`（x-share.ts・twitter.com intent・noopener）を再利用し要約＋現在 URL を本文に畳んで新規タブ）を出す。多軸状態は canonical URL に全部載るので URL を渡せば相手も同じ絞り込みを開ける。
- **保存した絞り込み（#139 段階3・実装済）**: discover のフィルタ状態を名前付きで保存し、ワンタップ切替する自分専用チャンネル（用語は「絞り込み」で統一・「ビュー」と呼ばない）。`{label, query}` を localStorage（`src/lib/feed/views.ts`・SSR 安全 getLS・配列順・同 query は二重登録しない・壊れ値は空配列に倒す）に保存し、`query` は **現在の filter の canonical 文字列（`serializeFilter`＝例 `tags=実生&sort=old`）を丸ごと**持つ＝多軸でもそのまま保存できる（views.ts は中身を解釈しない＝多軸化前の単一クエリ `#実生` も `parseFilterFromString` が後方互換で吸収）。UI `SavedViews`（チップ列＝`role=group`＋各チップ `aria-pressed`／先頭は「**絞り込みなし**」＝既定〔空 filter〕で active＝絞り込み解除・続いて保存分・保存は「今の絞り込みを保存」→名前入力・編集で × 削除）は **絞り込みパネルの最上部**に置く（#8/#9 指示＝絞り込み関連は展開エリア内・名前付きは絞り込みより上）。DiscoverGrid の**filter 反映経路に乗せて切替**（チップ tap → その query を `applyView`＝`parseFilterFromString` で filter に戻して適用・`pushState`・戻るで前の絞り込みへ）。active 判定は `serializeFilter` 一致（旧形式は `normalizeQuery` で canonical 化して同一視）。既定（空 filter）は保存できない（空 query 不可）。
- **タグ入力＝多段ドリルダウン＋検索（#22/#143/#144・実装済）**: 投稿時のタグは手打ちせず `TagPicker` から選ぶ。約 1,980 件の品種辞書を **カテゴリ→属→品種**（24 カテゴリ・近いもの順）の階層で持ち（`variety-catalog.ts`。**`TagPicker` を開いた時に動的 import で code-split**＝初期 composer バンドル非搭載・PWA precache でオフライン可。`variety-search.ts` は `catalog` を引数で受ける純関数でこの分割を保つ）、ドリルダウン（パンくず＋‹戻る・囲み外クリックで閉じる・入口は最上段）と全件横断のインクリメンタル検索（`foldForSearch`＝かな/カナ・大小・全半角を無視）で少クリック選択する。**タグと札を分ける（#181・#166 のタグ部分を撤回）**: タグ＝Twitter式ハッシュタグ＝**概要→詳細の全階層**を付けてよい。品種を選ぶと**`#属 #品種` の両方**を本文へ入れる（`findVarietyGenus` で属を引き、属→品種の順に `insertTag`・二重挿入防止で属は重複しない）。属の「このまま使う」は `#属` のみ。**カテゴリ（塊根植物/花木 等）はタグにしない**（ドリルダウンの見出し専用）。**品種チップには学名(`sci`)を小さく薄くイタリックで併記**（#200・投稿前に学名を確認でき #23「学名を厳密に書きたい層」に応える＋同名異植物の見分け。`SciName`・`aria-hidden`＝ボタンのアクセシブル名は `#品種名` のまま保つ。属/カテゴリ/世話記録タグには出さない・sci 無しのグループ概念は和名のみ）。札（鉢の名前＝具体1つ＝学名＋和名）はタグとは別概念（#182・`fuda.ts`＝`buildFuda` でハッシュタグから属＋品種を組む。`detectPlants`/`detect.ts` は撤去・§4 投稿詳細を参照）。集約は読み取り側の OR 検索（§6・別名横断）でも吸収できるが、属タグを直接付けることで属でも確実に辿れる。人気/最近で**属をタップしたら階層へ誘導**。選択済みタグは満たし色（緑）になり、**再タップで解除**（`tagsToUnpick`＝同属に兄弟が残らなければ属・カテゴリも連動して外す）。最近使ったは**投稿成功後**に本文のタグを記録（タップしただけは残さない）。世話/記録/特徴/仕立てのクイックタグ（`tag-catalog.ts`）は別枠で常設（特徴＝斑入り/綴化/石化 等・仕立て＝水耕/板付け 等は品種と別軸の自慢/栽培形態タグ・#221。分類語でないので #166 非抵触）。**入る値は本文 `#` テキストのみ＝`t` 化しない**（§6 契約不変）。旧 `PlantSuggest`/`childrenOf`（#63/#23 Phase2 の俗称検出・「もっと具体的に」）はこのドリルダウン＋検索に置換し撤去。クイックタグ（世話/記録/特徴/仕立て）は**頻度上位（先頭 `INLINE_LIMIT` 件）を常時インライン表示**し、あふれは行末「その他」ポップアップ（glass・×/Esc/囲み外で閉じる・**トリガーボタンに半分重なる縦位置で被せる**＝`relative` ラッパ内に `absolute top-1/2` で引き上げ・#187）で**あふれ分だけ**（先頭 `INLINE_LIMIT` 件を除いた残り）見せる＝幅で候補を消さず、かつインライン済みの定番は重複させない（#169/#186。発芽は世話へ移し 発芽→発根→実生 を隣接、記録から発芽を削除）。写真追加「＋」のポップアップ（ImagePicker compact）も同様にトリガーに半重ねで出す（#187）。タグ（品種）が無い植物は「追加をリクエスト」導線から **`/vote` の「品種への要望」板**（住民投票 BBS の先頭・Nostalgic）へ送る（#232・新規タブで開き合成中の下書きを保つ）。**旧 GitHub Issue 起票は廃止**＝一般ユーザーを GitHub に飛ばさない。品種に関する要望（並び順・追加・その他）はこの板に集約する。
