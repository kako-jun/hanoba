...
- **投稿詳細（拡大モーダル）**: 写真は**元の比率のまま**見せる（クロップしない・他者著作の改変回避 #61）。複数写真は横スクロールで確認できる。hanoba 自前投稿は 1:1 出力なので正方形に、他クライアントの非正方形はその比率で表示。flex 列の中で `flex-shrink` に潰されて横長化しないよう写真枠に **`shrink-0`** を付け、自然な高さを確保する（#108）＋著者（アイコン＋名前＋サイトリンク）＋一言＋認識植物＋反応。**本文は #タグ を除いて表示**（`stripHashtags`・タグはチップに出すのでフィードカードと挙動を揃える＝二重表示しない・#43）。**植物の「札」（鉢の名前・#182）は `buildFuda(post.hashtags, catalog)`** で組む＝`variety-catalog` の階層（カテゴリ→属→品種）から **最も具体的な「属 品種」を1枚**にする（例「パキポディウム ブレビカウレ」）。**カテゴリ（塊根植物 等）は札にしない・属に品種があれば属単独札は畳む**（タグは全階層だが札は具体1つ＝別概念・#181）。catalog はモーダル展開時に**動的 import**（初期フィードバンドルに載せない）。学名は `dictionary.ts` を属/品種名でルックアップして引けたら併記（#70 SciName）。札はハッシュタグと混同しないよう園芸の札（ラベル札）を模した形で出し、クリックで最も具体的な語の discover 検索へ。
