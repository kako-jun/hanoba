...
- **投稿頻度の高い人を「薄める」（#138）**: 人ごとに **1/2・1/5・1/10** の間引き度合いを設定でき、その人の投稿をフィードで決定的に間引いて表示する（ミュートの手前の柔らかい手段）。判定は `hash(post.id) % level === 0`（FNV-1a・id 由来）＝**deterministic**でリロードしても残る投稿は同じ・相対順序は保つ。状態（pubkey→level）は localStorage（`src/lib/feed/dilution.ts`・SSR 安全 getLS・純関数 `diluteFeed(posts, map)`）。適用は取得済みリスト→表示の間（`PostGrid`＝フィード/discover 共通）に1段挟む。**`/me`（自分の投稿）には掛けない**＝自分を薄めない。UI は投稿詳細モーダルの著者ヘッダ下に **なし/1/2/1/5/1/10** の離散セグメント（`DilutionControl`・radiogroup）で置き、設定すると即フィードに反映（同タブはカスタムイベント、別タブは storage で同期）。
