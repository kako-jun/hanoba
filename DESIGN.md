...
- **タグ入力＝多段ドリルダウン＋検索（#22/#143/#144・実装済）**: 投稿時のタグは手打ちせず `TagPicker` から選ぶ。約 1,400 件の品種辞書を **カテゴリ→属→品種**（15 カテゴリ・近いもの順）の階層で持ち（`variety-catalog.ts`。**`TagPicker` を開いた時に動的 import で code-split**＝初期 composer バンドル非搭載・PWA precache でオフライン可。`variety-search.ts` は `catalog` を引数で受ける純関数でこの分割を保つ）、ドリルダウン（パンくず＋‹戻る・囲み外クリックで閉じる・入口は最上段）と全件横断のインクリメンタル検索（`foldForSearch`＝かな/カナ・大小・全半角を無視）で少クリック選択する。品種を選ぶと**その品種タグだけ**を本文へ入れる（1鉢1札・#166）。属の「このまま使う」は `#属` のみ。**カテゴリはタグにしない**。より具体的な品種を入れたら本文の上位（属）タグは外す（`ancestorTagsPresent`）。集約は読み取り側の OR 検索（§6・別名横断）で吸収するので葉だけでも属から辿れる。人気/最近で**属をタップしたら階層へ誘導**。選択済みタグは満たし色（緑）になり、**再タップで解除**（`tagsToUnpick`＝同属に兄弟が残らなければ属・カテゴリも連動して外す）。最近使ったは**投稿成功後**に本文のタグを記録（タップしただけは残さない）。世話/記録のクイックタグ（`tag-catalog.ts`）は別枠で常設。**入る値は本文 `#` テキストのみ＝`t` 化しない**（§6 契約不変）。旧 `PlantSuggest`/`childrenOf`（#63/#23 Phase2 の俗称検出・「もっと具体的に」）はこのドリルダウン＋検索に置換し撤去。世話/記録のクイックタグは**頻度上位（先頭 `INLINE_LIMIT` 件）を常時インライン表示**し、あふれは行末「その他」ポップアップ（glass・×/Esc/囲み外で閉じる）で**全件**見せる＝幅で候補を消さず定番を共通化する（#169。発芽は世話へ移し 発芽→発根→実生 を隣接、記録から発芽を削除）。タグが無い植物は「追加をリクエスト」導線（当面は hanoba Issue 起票・将来 #163/#164 へ差し替え）から起票する。
