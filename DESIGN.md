...
- **人気ランキング（#162・`/ranking`）**: 植物**品種**の人気を音楽チャート風に出す。**バックエンドレス**（§6・集計サーバを持たない）で、取得済みの **`t:hanoba` 投稿だけ**（`fetchRankingPosts`・既定 limit 500・画像ありのみ＝フィードと同母集団）を**クライアントで数える**「読むたびに数える」方式。母集団を `t:hanoba` 限定にするのは、**自分の投稿が見えてチャートを動かす＝投稿の動機**にするため（discover の横断集約は混ぜない）。集計と差分は純粋ロジック `src/lib/feed/ranking.ts`（relay も `Date.now()` も持たず `now` を引数で受ける＝テスト可能）に集約し、島 `RankingBoard` は取得・`now=Math.floor(Date.now()/1000)` の供給・描画だけを担う。
  ...
  - **先週比（`rankWithDeltas`）**: 「直前の週」＝現在週より前で**データのある直近の過去週**（カレンダー連続でなくてよい）。**NEW**＝現在週より前のどの週にも一度も載っていない。**RE**＝過去に載ったが直前週には不在で今週復帰。**↑N/↓N/—（same）**＝直前週との順位差。**直前の過去週が無い（初週）なら全行 NEW**（偽の矢印を出さない）。
  - **途中経過（変動）チャート（#162・uPlot）**: 順位の**表**の下に、上位約 6 品種の週次票数の推移を **1枚の折れ線チャート**で重ねる（x＝ISO 週・古い→新しい、y＝その週の票数＝整数、1品種1線・凡例＝和名）。維持者方針「変動はグラフなら uPlot のような軽量 lib を使う」に従い、**軽量・MIT・依存ゼロ・canvas の uPlot** を採用（純データは `rankRunData(posts, catalog, keys)`＝全週に整列した票数マトリクス・出現の谷は 0 で埋め線を連続させる・テスト可能）。uPlot は `window`/canvas に触れるので **クライアント専用＝マウント後に動的 import**（SSG ビルド中に走らせない）し、`RankRunChart` 自体も `React.lazy` で**遅延ロード**（チャートを出すときだけ uplot チャンクを取得・初期描画を軽く）。暗いシック地に合わせ軸/グリッド/ラベルはパレットの ink、線色は緑/オレンジ/黄/明るい葉/ink の少数セット（重い塗りはしない）。canvas は装飾なので `aria-hidden`＝**意味は上の表（順位・品種・件数・先週比）が正本**。図には説明的 `aria-label` を付ける。reduced-motion: uPlot は既定で非アニメーション。**週が2つ未満なら退化グラフを描かず**「推移グラフは週が2つ以上たまると表示されます。」の注記に差し替える（潰れた線を見せない・RankingBoard 側でも同条件で gate）。
