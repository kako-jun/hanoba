...
- **名前付きビュー（#139 段階3・実装済）**: discover の検索状態を名前付きで保存し、上部チップでワンタップ切替する自分専用チャンネル。ビュー＝`{label, query}` を localStorage（`src/lib/feed/views.ts`・SSR 安全 getLS・配列順・同 query は二重登録しない・壊れ値は空配列に倒す）に保存し、`query` は **現在の `?q=` 文字列を丸ごと**持つ＝多軸 URL（#131）が来てもそのまま活きる**前方互換**（views.ts は中身を解釈しない）。UI `SavedViews`（チップ列＝`role=group`＋各チップ `aria-pressed`＝`FilterChips` に揃える。tabpanel 紐付け・roving tabindex を持たないため tab 契約は満たせない／「すべて」＝`?q=` 無し＋保存ビュー・active は現在 `?q=` 一致・保存/切替/削除。並べ替え UI は段階的に未実装）は DiscoverGrid の**既存 `?q=` 反映経路に乗せて切替**（チップ tap → その query を `applyView` で適用＝`pushState`・戻るで前のビューへ／「すべて」は `?q=` を消して既定検索へ）。既定検索（`?q=` 無し＝すべて）は名前付きビューにできない（空 query は保存不可）＝多軸 URL（#131）が来ても空＝すべての前方互換。多軸フィルタ UI は #131 待ち。
