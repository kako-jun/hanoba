...
- **人気ランキング（#162・`/ranking`）**: 植物**品種**の人気を音楽チャート風に出す。**バックエンドレス**（§6・集計サーバを持たない）で、取得済みの **`t:hanoba` 投稿だけ**（`fetchRankingPosts`・既定 limit 500・画像ありのみ＝フィードと同母集団）を**クライアントで数える**「読むたびに数える」方式。母集団を `t:hanoba` 限定にするのは、**自分の投稿が見えてチャートを動かす＝投稿の動機**にするため（discover の横断集約は混ぜない）。集計と差分は純粋ロジック `src/lib/feed/ranking.ts`（relay も `Date.now()` も持たず `now` を引数で受ける＝テスト可能）に集約し、島 `RankingBoard` は取得・`now=Math.floor(Date.now()/1000)` の供給・描画だけを担う。
  ...
  - **sparkline（#162）**: 上位約10件に週次票数の推移を軽量インライン SVG polyline で添える。装飾なので `aria-hidden`＝意味は行の `aria-label`（順位・品種・件数・先週比）が持つ。
  ...
  - **開発専用プレビュー（`?demo`）**: `import.meta.env.DEV` **かつ** URL に `?demo` のときだけ、合成 fixture（`src/lib/feed/ranking.demo.ts`・複数週/差分/sparkline を作り込んだダミー）を使う＝`npm run dev` でリレーに**何も publish せず**リッチ状態を確認できる。**本番ビルドでは `import.meta.env.DEV=false` でこの分岐ごとデッドコード化**（fixture も banner 文字列もバンドルに残らず、本番ページは捏造データを一切出さない）。
